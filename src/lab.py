...
╚══════════════════════╝       
          """)
    textbox(
        "welcome to MAKISE'S LAB! I can make your monster to become stronger than ever! choose the one you'd like to upgrade by typing its [number] or leave the lab by typing [exit]")
    while True:
        ...
        for i in range(len(inv_monster_data)):
            if inv_monster_data[i]["user_id"] == user_id:
                ...
                no += 1
        print(current_user_monsters)
        choice = input("➤ ID/EXIT ")
        if choice.upper() == "EXIT":
            break
        elif exist(current_user_monsters, choice, "number"):
            monster_upgrade_index = 0
            for i in range(len(inv_monster_data)):
                if inv_monster_data[i]['user_id'] == user_id and inv_monster_data[i]['monster_id'] == current_user_monsters[int(choice)-1]['current_monster_id']:
                    monster_upgrade_index = i
            if int(inv_monster_data[monster_upgrade_index]["level"]) < 5:
                ...
                if user_money >= cost:
                    while True:
                        textbox(
                            f"currently you have {user_money} OC. its going to cost you {cost} OC, are you sure you want to upgrade this monster?")
                        confirm = input("➤ Y/N: ")
                        if confirm.upper() == "Y":
                            inv_monster_data[monster_upgrade_index]["level"] = str(
                                int(inv_monster_data[monster_upgrade_index]["level"]) + 1)
                    ...
                    textbox(
                        f"you only have {user_money} oc in your wallet... you need {cost} oc to upgrade that monster. just [exit] if you don't have money...")
            else:
