...
def monster_shop(monster_data, shop_monster_data, inv_monster_data, users_data, user_id):
    textbox(
        "Imported directly from ragunan! type the [ID] to choose your desired monster and type [back] to get back to main shop")
    print()
    while True:
        ...
        if buy.upper() == "BACK":
                ...
                while True:
                    if int(display_monster_shop[index(display_monster_shop, int(buy), "id")]['stock']) < 1:
                        ...
                        if int(users_data[index(users_data, user_id, 'id')]["oc"]) >= int(
                            ...
                            while True:
                                textbox(
                                    f"you currently have {users_data[index(users_data, user_id, 'id')]['oc']} oc and the price is {shop_monster_data[index(shop_monster_data, int(buy), 'monster_id')]['price']} oc, are you sure you want to buy this? [y/n]")
                                confirm = input("➤ ")
                                if confirm.upper() == "Y":
                                    ...
                                    shop_monster_data[index(shop_monster_data, int(buy), 'monster_id')]['stock'] = str(
                                        int(shop_monster_data[index(shop_monster_data, int(buy), 'monster_id')][
                                                'stock']) - 1)
                                    users_data[index(users_data, user_id, 'id')]["oc"] = str(
                                        int(users_data[index(users_data, user_id, 'id')]["oc"]) - int(
                                            shop_monster_data[index(shop_monster_data, int(buy), 'monster_id')][
                                                'price']))
                                    textbox(
                                        f"thank you for your patronage! you currently have {users_data[index(users_data, user_id, 'id')]['oc']} oc left in your wallet")
                                    break
                                elif confirm.upper() == "N":
                                    break
                        else:
                            textbox(
                                f"you only have {users_data[index(users_data, user_id, 'id')]['oc']} oc lmao, come back when you have the money")
                            break
                    break
            else:
            ...
            textbox(
                "let me repeat myself, choose one of the monster i have by typing the [ID] of the monster or get back to the main shop by typing [back]")
                ...
                while True:
                    ...
                    if confirm.upper() == "Y":
                        ...
                        for i in range(len(inv_item_data)):
                            if (inv_item_data[i]['user_id'] == user_id) and (
                                    inv_item_data[i]['type'] == display_item_shop[int(buy) - 1]['type']):
                                inv_item_data[i]['quantity'] = str(int(inv_item_data[i]['quantity']) + 1)
                                include = True
                        if not include:
                            inv_item_data.append(
                                {"user_id": user_id, "type": display_item_shop[int(buy) - 1]['type'], "quantity": 1})
                        shop_item_data[int(buy) - 1]["stock"] = str(int(shop_item_data[int(buy) - 1]["stock"]) - qty)
                        display_item_shop[int(buy) - 1]["stock"] = str(
                            int(display_item_shop[int(buy) - 1]["stock"]) - qty)
                        users_data[index(users_data, user_id, 'id')]["oc"] = str(
                            int(users_data[index(users_data, user_id, 'id')]["oc"]) - int(
                                shop_item_data[int(buy) - 1]['price']))
                        break
                    elif confirm.upper() == "N":
                        break
            else:
