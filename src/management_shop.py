...
def add(monster_data: list[dict],
    ...
    if choice.lower() == "monster":
        ...
        all_potions = ["strength", "resilience", "healing"]
        unsold_potions =[]
...
def edit(monster_data: list[dict],
    ...
    if choice.lower() == "monster":
        ...
        if newstock and not newprice:  # only change stock
            ...
            shop_item_data[itemid - 1]['price'] = newprice
            print(
                f"{monster_data[itemid - 1]['type']} has been restocked with a quantity of {newstock} and has been updated with a price of {newprice}!")
    return shop_item_data, shop_monster_data
...
def shop_management(
    ...
    for i in range(len(shop_monster_dat)):
        if exist(monster_dat, shop_monster_dat[i]['monster_id'], "id"):
    ...
    while True:
        ...
        if option.lower() == 'view':
            ...
            shop_item_dat, shop_monster_dat = add(monster_dat, shop_item_dat, shop_monster_dat, display_item)
